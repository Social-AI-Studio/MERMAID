"""
 ...
 """
 # https://github.com/huggingface/transformers/blob/main/examples/research_projects/visual_bert/modeling_frcnn.py
...
# other:
def norm_box(boxes, raw_sizes,targetdevice):
    if not isinstance(boxes, torch.Tensor):
        normalized_boxes = boxes.copy().to(targetdevice)
    else:
        normalized_boxes = boxes.clone().to(targetdevice)
        
    size1 = raw_sizes[:,1].to(targetdevice)
    size0 = raw_sizes[:,0].to(targetdevice)
    normalized_boxes[:, :, (0, 2)] /= size1
    normalized_boxes[:, :, (1, 3)] /= size0
...
class GeneralizedRCNN(nn.Module):
    ...
    def inference(
        ...
        **kwargs,
        ...
        normalized_boxes = norm_box(boxes, original_sizes,self.device)
        return OrderedDict(
            {
                "obj_ids": classes,
